...
traintest.inp <- missForest(rbind(train[,-7], test))

i.train <- traintest.inp$ximp[1:L,]
i.train$Party <- train$Party
i.test  <- traintest.inp$ximp[(L+1):nrow(traintest.inp$ximp),]
...
write.csv(MySubmission, "SubmissionSimpleLog13.csv", row.names=FALSE,quote = FALSE)
